dojo.mixin(trst.task.acc,{
  delivery_note: {
    ...
    init: function(){
      // Validate presence of client and transporter and connect button 'Post'
      ...
      if (dojo.query('input.freight').length > 0){
        dojo.query('input.freight').forEach(function(fr,i){
          ...
          trst.task.acc.delivery_note.initFreightSearch(fr,i)
        })
      }
      ...
      if (triggers.length > 0){
        triggers.onchange(function(){
          trst.task.acc.delivery_note.calculator()
        })
      }
      ...
      if (drows.length > 0){
        drows.onclick(function(e){
          trst.task.acc.delivery_note.deleteRow(e.target)
        })
      }
    ...
    validateRequired: function(){
      ...
      dojo.query('.button.post.mc-evt').onclick(function(){
        if (selectedValue(cl) == 'null'){
          alert('Nu aţi selectat destinatarul!')
        }
        else if(selectedValue(tp) == 'null'){
          alert('Nu aţi selectat transportatorul!')
        }
        else{
          trst.task.init(trst.task.acc.hd.task_id,'post','new?client_id=' + selectedValue(cl) + '&transporter_id=' + selectedValue(tp))
        }
      })
    },
    initFreightSearch: function(fr,i){
      var search = new dijit.form.FilteringSelect({
        ...
        value: "",
        store: trst.task.acc.delivery_note.freightStore,
        searchAttr: "label",
        placeHolder: "Selectare material",
        labelAttr: "label",
        onChange: function(id){
          trst.task.acc.delivery_note.onSelectFreight(search)
        }
      },fr.id)
    },
    ...
    connectButtons: function(){
      ...
      if (bs.length > 0){
        bs.onclick(function(){
          trst.task.acc.delivery_note.calculator()
          if (trst.task.acc.delivery_note.validateInputs()){
